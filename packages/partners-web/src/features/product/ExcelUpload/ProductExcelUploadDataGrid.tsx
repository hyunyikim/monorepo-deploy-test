...
import {PRODUCT_EXCEL_COLUMN, PRODUCT_EXCEL_INPUT} from '@/data';
...
function ProductExcelUploadDataGrid({
	inputs,
	fields,
	data,
	excelErrors,
	...
	return (
		<>
					...
					{excelErrors && excelErrors?.size > 0 && (
						<>
							<MuiButton
								...
								data-tracking={`itemadmin_excelpublish_infodeficiency_click,{'button_title': '상품 정보부족 버튼 클릭'}`}>
								상품 정보 부족{' '}
			...
			<Table
					...
					<>
						...
						{fields.map((field, idx) => (
							<HeadTableCell
								key={`head_${idx}`}
								required={
									!!PRODUCT_EXCEL_INPUT[field.key]?.required
								}>
				...
				{data.map((row: ProductExcelUploadInputWithCustomFields, i) => {
					return (
						<TableRow key={`row_${i}`}>
...
export default ProductExcelUploadDataGrid;
