...
function ServiceInterworkCafe24() {
	...
	useEffect(() => {
		// 이미 연동된 사용자는 연결 설정 페이지로 이동
		if (value) {
			goToParentUrl('/b2b/interwork/cafe24');
		}
	}, [value]);

	const onClickConnectCafe24Button = useCallback(() => {
		...
		<Stack
			...
			<ServiceInterworkDetailTitle
				...
				Button={
					<Button
						...
						onClick={onClickConnectCafe24Button}>
						연동하기
					</Button>
				}
			/>
			<ServiceInterworkDetailContent
				...
				<>
					...
					<Typography>
						최초 카페24 연동 시, ‘배송완료’ 시점에 ‘전체상품’에
						대해서 개런티가 발급됩니다.
						<br />
						개런티 발급 시점, 발급 상품, 알림톡 발송 설정을 모두
						변경할 수 있습니다.
						<br />
					</Typography>
					...
					<Typography>
						...
						<br />
						3. 연동이 완료되면, 신규 주문 건에 대해서 배송완료
						처리하면 자동으로 개런티가 고객에게 발급됩니다.
					</Typography>
					<Typography variant="h4">개런티 발급 시점</Typography>
					<Typography>
						[배송완료] <span className="arrow">→</span>
						카페24 관리자 페이지에서 배송완료 처리 시, 자동으로
						개런티 발급 (알림톡 발송)
						<br />
						[수동발급] <span className="arrow">→</span> 카페24
						관리자 페이지에서 배송완료 처리 시, 버클 파트너스 개런티
						목록에 “신청대기” 상태로 저장
						<br />
					...
					<Typography>
						[전체상품] <span className="arrow">→</span> 카페24에
						등록된 모든 상품에 대해서 개런티 발급
						<br />
						[카테고리] <span className="arrow">→</span> 카페24에
						등록된 카테고리 중에서 선택한 카테고리 상품만 개런티
						발급
					</Typography>
					...
					<Typography>
						[발송함] <span className="arrow">→</span> 신규 주문완료
						시, 자동으로 고객에게 알림톡 발송
						<br />
						[발송안함] <span className="arrow">→</span> 신규
						주문완료 시, 알림톡 미발송
					</Typography>
				</>
			</ServiceInterworkDetailContent>
