...
function SubscribeNotice({sx = {}}: Props) {
	...
	const SUBSCRIBE_NOTICE: SubscribeNoticeType = useMemo(
		() => ({
			TRIAL: (
				<>
					<Stack>
				...
				<>
					<Stack>
						...
						<Typography fontSize={13} color="grey.900">
							고객이 브랜드(기업)에서 상품을 구매할 경우, 해당
							상품에 대한 디지털 개런티를 고객에게 발급할 수
							있습니다. 발급량은 요금제 별로 디지털 개런티를
							발급이 가능한 수를 의미합니다. 요금제를 선택할때에는
							브랜드(기업)의 평균적으로 판매량을 고려해 선택해
							보세요.
						</Typography>
					</Stack>
					<Link
			...
			CHANGE_PLAN_MONTH_TO_YEAR: (
				<>
					<Stack>
						...
						<Typography fontSize={13} color="grey.900">
							월결제를 연결제로 변경 하는 경우, 월결제가 끝난
							다음달 결제일에 연결제 요금이 결제됩니다.
							<br />
			...
			CHANGE_PLAN_YEAR_TO_MONTH: (
				<>
					<Stack>
						...
						<Typography fontSize={13} color="grey.900">
							연결제 이용 시 월결제로 변경 할 경우에는 연결제가
							끝나는 다음 결제일에 월결제 요금이 결제되며, 자세한
							사항은 아래 구독가이드를 참고해주세요. (단, 월
							요금제로 바로 변경을 원하신다면{' '}
							<Link
								className="cursor-pointer"
				...
				<>
					<Stack>
						...
						<Typography fontSize={13} color="grey.900">
							현재 플랜의 다운그레이드는{' '}
							<b>월결제 이용시에만 가능</b>
							하며, 변경시 다음달 결제일부터 새로운 플랜이
							적용됩니다. (연결제 이용 고객께서 다운그레이드가
							필요할 경우,{' '}
							<Link
								className="cursor-pointer"
								onClick={openChannelTalk}>
