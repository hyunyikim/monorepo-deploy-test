...
function SetupGuarantee() {
	...
	const maximumAdditionalCategory = b2bType === 'brand' ? 8 : 5;
	const requiredCategoryLength = b2bType === 'brand' ? 1 : 2;
	...
	// const {setData} = useGetPartnershipInfo((state) => ({
	...
	const handleFormData = () => {
		...
		const values = getValues();
		const fillteredExampleList = exampleList.filter((el, idx) => {
			if (idx >= 2) {
				return el;
			}
		});
		...
		/* 커스텀 정보 */
		formData.append('nftCustomField', fillteredExampleList.join(','));
	...
	useEffect(() => {
		if (b2bType === 'brand') {
			setExampleList([
				...brandCategoryRequiredList,
				...categoryExampleList[tabState],
			]);
		} else {
			setExampleList([...categoryRequiredList]);
		}
	}, [tabState]);
